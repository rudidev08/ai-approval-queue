...
- reminders.py — the reminders area; reads the lists and open reminders through the host app's reminders server every minute in its own thread, so a slow read never holds a page poll
